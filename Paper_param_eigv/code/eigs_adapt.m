function [V, D, R, permB, flag] = eigs_adapt(varargin)
% EIGS code waarbij ook de subspace wordt teruggegeven
...
if isrealprob && ishermprob
    ...
    if ~ismember(innerOpts.whch,{'LM', 'SM', 'LA', 'SA', 'BE'})
        error(message('MATLAB:eigs:EigenvalueRangeNotValidSym'));
    end
else
    ...
    if ~ismember(innerOpts.whch,{'LM', 'SM', 'LR', 'SR', 'LI', 'SI'})
        error(message('MATLAB:eigs:EigenvalueRangeNotValidComp'));
    end
end
...
% Nested functions in CallARPACK
    function cols = checkIpntr
        ...
        if ~all(rows==1)
            error(message('MATLAB:eigs:ipntrMismatchWorkdColumn'));
        end
    end
...
if mode == 1
    innerstr = getString(message('MATLAB:eigs:PrintTimingsComputeAX',sprintf('%f',timings(3))));
else % mode = 3
    if isempty(B)
        innerstr = getString(message('MATLAB:eigs:PrintTimingsSolveASIGMAI',sprintf('%f',timings(3))));
    else
        innerstr = getString(message('MATLAB:eigs:PrintTimingsSolveASIGMAB',sprintf('%f',timings(3))));
    end
end
if mode == 3 && Amatrix
    if isempty(B)
        prepstr = getString(message('MATLAB:eigs:PrintTimingsPreproSigmaI',sprintf('%f',timings(1))));
    else
        prepstr = getString(message('MATLAB:eigs:PrintTimingsPreproSigmaB',sprintf('%f',timings(1))));
    end
else
    prepstr = getString(message('MATLAB:eigs:PrintTimingsPreprocessing',sprintf('%f',timings(1))));
end
sstr = getString(message('MATLAB:eigs:PrintTimingsCPUTimingResults'));
postpstr = getString(message('MATLAB:eigs:PrintTimingsPostprocessing',sprintf('%f',timings(4))));
totalstr = getString(message('MATLAB:eigs:PrintTimingsTotal',sprintf('%f',timings(5))));
fprintf(['\n' sstr '\n' ...
    prepstr ...
    'IRAM/IRLM:                                 %f\n' ...
    innerstr ...
